...
**Query Parameters:**
- `lang` (optional) — Idioma: `pt-br` | `en-us` | `es-es` (padrão: `pt-br`)
...
**Response 200 (após usar créditos):**
```json
{
  "availableCredits": 7,
  "totalCredits": 15,
  "planType": "free",
  "lastCreditRefillAt": "2025-01-10T15:42:30Z"
}
```
...
|-------|------|-----------|
| `availableCredits` | Integer | Créditos disponíveis para usar |
| `totalCredits` | Integer | Total de créditos acumulados (histórico) |
| `planType` | String | Tipo de plano: `"free"` ou `"premium"` |
| `lastCreditRefillAt` | DateTime \| null | Última data de reabastecimento de créditos |
...
- **Plano Free:** 10 créditos iniciais
- **Análise IA:** Custa 3 créditos por análise
- **Anúncios:** Ganham créditos conforme tipo:
  - Rewarded Video: 10 créditos
  - Interstitial: 5 créditos
  - Banner: 1 crédito
- **Como recarregar:** Assista anúncios ou faça upgrade para plano Premium
...
**Fields:**
- `habitId` (string, CUID format, required) — ID do hábito a analisar (formato CUID: 24+ caracteres alfanuméricos)
...
**⚠️ Nota sobre IDs:**
> O campo `habitId` aceita IDs no formato **CUID** (Collision-resistant ID). Exemplos válidos: `clw7g8h0000002np7b8b8b8b`, `cmk6ugw36000...`. Use o `id` retornado ao criar/listar hábitos.
